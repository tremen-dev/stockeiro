...
function setBuildChannel(commit: string, environment: string, builtAt: string) {
...
describe('SPEC-031 CA-1: el cuerpo es exactamente el contrato', () => {
  ...
  it('el conjunto de claves es EXACTAMENTE { commit, environment, builtAt }', async () => {
    setBuildChannel(SHA_A, 'production', ISO_A);
    const { GET } = await loadRoute();
    ...
    expect(Object.keys(body).sort()).toEqual(['builtAt', 'commit', 'environment']);
    expect(body).toEqual({ commit: SHA_A, environment: 'production', builtAt: ISO_A });
  });
  ...
  it('sin nada en el canal de build responde 200 igualmente, con las tres claves en `unknown`', async () => {
...
describe('SPEC-031 CA-2: la identidad va congelada con el artefacto', () => {
  it('mutar process.env entre dos peticiones NO cambia la respuesta', async () => {
    ...
    setBuildChannel(SHA_B, 'preview', ISO_B);
