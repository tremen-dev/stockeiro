...
| CA-15 — los avisos de diagnóstico no se rompen en formato tarjeta (`34ch`, envuelven, completos) | `src/app/globals.css` › `.tarjeta-estado .estado-caja`: **no** sobrescribe el `max-width: 34ch` de `.quote-*`, al revés de lo que hace `.data-table .estado-caja` | `tests/e2e/tarjetas-geometria.spec.ts` › «CA-15: los avisos conservan su caja, envuelven y se leen enteros» (texto idéntico al de 1280, `max-width` distinto de `none`, ninguna línea por encima de la caja, el más largo envuelve, ninguno parte palabra, y ninguno se sale de su tarjeta) | **✅ Cerrado con la redacción de la ronda 6, y medido esta ronda con el módulo compartido `tests/e2e/geometria.ts`** (instrucción del humano del 2026-08-24: reutilizar el módulo, no un arnés propio). La letra nueva se corresponde **cláusula a cláusula** con lo que la guardia afirma (`tests/e2e/tarjetas-geometria.spec.ts:708-824`), y las **seis** propiedades salen verdes medidas por mí, a **360 y 390 px en las dos pantallas**: **(1) completo, carácter a carácter** — `allTextContents()` a 360/390 **idéntico** al de 1280 en los cuatro pares (guardia: `.toEqual(ancho1280)`). **(2) caja acotada = 34ch, nunca `none`** — `medirPropiedadesComputadas` da `max-width` **270.504px** en `/vigiladas` y **244.8px** en `/cartera`, a los dos anchos; un `34ch` resuelto con sonda bajo la misma fuente computada da **270.5** (Geist 12px) y **244.797** (Geist Mono 12px): la caja **es** los 34ch, no sólo «distinta de `none`». **(3) ninguna línea se extiende por encima de la caja** — `medirIntegridadDePalabra` (M3 del módulo), `anchoLineaMax` contra el `max-width` computado, **comparado sin restar nada**: cabe en los 5 avisos × 2 anchos, con márgenes **3,30 / 10,33 / 31,57** px (`/vigiladas`) y **61,66 / 64,80** px (`/cartera`). **El ✅ no depende de la tolerancia** — ver la salvedad (i). **(4) el aviso de más palabras de cada pantalla ocupa más de una caja de línea** — `/vigiladas`: `.quote-fail`, **16 palabras → 2 líneas**; `/cartera`: `.quote-fail`, **13 palabras → 3 líneas**; a los dos anchos. Es exactamente el `reduce` por `palabras` + `.toBeGreaterThan(1)` de la guardia. **(5) ninguno parte palabra (M3)** — `lineas ≤ palabras` en los 10 casos (2/16, 2/10, 1/8, 3/13, 2/8). **(6) ninguno desborda su tarjeta**, las **dos** aserciones que el CA nombra — `medirDesbordePorElemento` rooteado en `${tarjetas} > li`: **241** medidos en `/vigiladas` y **82** en `/cartera`, **0 violaciones**; y `medirCajas`: el `right` de cada aviso (303,50 / 307,50 / 327,00 / 353,00) dentro del de su tarjeta (344 / 370), **0 fuera**. `F-VERIF-054-2` **cerrado**: la cláusula autocontradictoria ya no está, y la que la sustituye es verificable y está verificada. ⚠️ **Dos salvedades de guardia, ninguna bloquea el ✅ y las dos van al arquitecto.** **(i) ADR-035 §1 contra la guardia**: el ADR —**aprobado** hoy— nombra literalmente *«un ancho contra los 34ch de `.quote-*`»* como **medida contra umbral declarado**, clase a la que *«no se le resta nada»*; la guardia compara `anchoLineaMax <= tope + TOLERANCIA_PX`. **Hoy no decide nada, y lo he comprobado en vez de suponerlo**: la propiedad se cumple **sin** la tolerancia, con 3,30 px de margen en el caso más apretado. Es material de `F-ADR-035-1`, y ese follow-up **se queda corto**: ADR-035 §2 sólo baja la regla a `medirAreaTactil` y `medirSuelosTipograficos`, los dos **en el módulo**, y este tercer caso vive **en una guardia**. **(ii) la guardia afirma menos que el CA en la cláusula (2)**: el CA dice *«`max-width: 34ch` computado, nunca `none`»* y la guardia sólo asegura `.not.toBe('none')`, así que un futuro `max-width: 60ch` pasaría en verde. La propiedad **la he medido yo y se cumple**; lo que no está blindado es la regresión. Es la misma clase de hueco que `F-SPEC-054-9`, y lo dejo como **`F-SPEC-054-10`**. *(Comprobado además que la aserción (3) no es vacua por su propio selector: el `topes.get(t.texto.slice(0,60))` **casa en los 10 casos** —ni un fallo de clave—, así que se ejecuta de verdad.)* | ✅ |
| CA-16 — cero regresión funcional: suites enteras verdes y diff acotado (CE-5) | No hay implementación: es el resultado. El diff **no toca** `src/db/`, `drizzle/`, `src/lib/portfolio/`, `src/lib/watchlist/zone-status.ts` ni `src/lib/market/` | `npx vitest run` → **1702/1702** en 112 ficheros · `npx playwright test` → **323/323**. Detalle de los seis ficheros que sí se tocaron y por qué, en §Decisiones y hallazgos | **✅ Cerrado con la redacción de la ronda 6, incluidas la letra (f) y la cláusula de exhaustividad.** **(a) Suites enteras, corridas por mí esta ronda**: `npx vitest run` **1702/1702** en 112 ficheros; `npx playwright test` **323/323** (4,7 min); `npm run typecheck` exit 0; `npm run lint --max-warnings=0` exit 0; `APP_BASE_URL=http://localhost:3200 npm run build` exit 0. **(b) Diff acotado a los commits de esta spec**, como el CA pide: `merge-base(origin/main, HEAD) = b9080be`, y los **19** commits desde ahí son **todos de SPEC-054** (`git log b9080be..HEAD`), así que hoy el acotado y el `origin/main...HEAD` coinciden. No toca `src/db/`, `drizzle/`, `src/lib/portfolio/`, `src/lib/watchlist/zone-status.ts` ni `src/lib/market/`; los únicos ficheros de `src/` son los **seis** de las dos pantallas. **(c) La letra (f), probada por aritmética como el CA exige**: `git diff --numstat b9080be...HEAD -- tests/e2e/geometria.ts` → **`630  0`**. Cero líneas borradas, y `export const TOLERANCIA_PX = 1;` está en la **línea 79** tanto en la base como en HEAD (comparado con `git show`). **(d) Las dos listas SON exhaustivas — lo conté**: `git diff --name-status b9080be...HEAD -- tests/` da **16 ficheros**, exactamente **9 `M` + 7 `A`**. Los 9 modificados son uno a uno los que el CA clasifica: `spec046.ts` (a), `vigiladas-capa-edicion.spec.ts` (b), `vigiladas-editar.spec.ts` (a), `geometria-rutas.spec.ts` (d)+(c), `movil-alta.spec.ts` (a), `geometria-puntos-ciegos.spec.ts` (c), `spec044-frontera.test.ts` y `spec043-sin-refrescar.test.ts` (e), y `geometria.ts` **(f)**. Los 7 añadidos son los 7 que el CA declara nacidos aquí, y comprobé con `git cat-file -e b9080be:<fichero>` que **ninguno existía** en la base. **Ni un fichero de test tocado y sin clasificar: el hueco de la ronda 5 está cerrado.** **(e) Los cuatro «sin tocarse»** (`vigiladas-orden`, `cartera`, `decimales`, `sin-refrescar-geometria`) **no aparecen en el diff**. **(f) Releí los diffs, no sólo el recuento**: todas las líneas borradas de los 8 ficheros con borrados son localizadores, esperas, mensajes de fallo y anchos. El (c) de `geometria-puntos-ciegos.spec.ts` lo leí entero — se mueve de 360 a **730 px** y **las tres precondiciones y la conclusión siguen literalmente ahí** (`.toBe('auto')`, `.toBeGreaterThan(caja.visible)`, `.toBeGreaterThan(caja.ventana + TOLERANCIA_PX)`, M1 a 0). **Cero aserciones quitadas o debilitadas, cero tolerancias subidas, cero `skip`/`fixme`, ninguna aserción sustituida por una espera, ningún conjunto de anchos recortado para esquivar un rojo.** | ✅ |
...
| CA-21 — la salida prohibida no se usa: ni un `overflow: hidden` nuevo (R-2 de EPIC-007) | `src/app/globals.css` §SPEC-054: ni una regla de `overflow` nueva | `tests/e2e/tarjetas-geometria.spec.ts` › «CA-21: ni un `overflow: hidden` nuevo, y los que hay son los de siempre» (inventario en tiempo de ejecución contra los heredados) · `tests/spec054-breakpoint-y-rutas.test.ts` › dos casos sobre la fuente · **mutación**: un `overflow-x: hidden` en `.tarjeta > *` lo pone rojo | **✅ Se sostiene: la aserción no cambió en la ronda 6, y la volví a comprobar en vez de heredarla.** El inventario en tiempo de ejecución lo produce la guardia en mi pasada completa, y `_qa/SPEC-054/overflow-recortado.txt` volvió **byte a byte idéntico** al commiteado: los únicos que recortan a 360 y 390 son `html`, `body` **(i)**, `table.data-table` **(ii)** y, en `/cartera`, dos `form.card.auth-form` **(iii)** — **exactamente** las tres familias enumeradas, ni una más. En la fuente, el diff de `globals.css` no añade ni una declaración de `overflow`. Y sobre `F-SPEC-054-9`, que la ronda 6 escribe en el CA: lo **verifiqué**, no lo di por bueno — `HEREDADOS = /^html\b|^body\b|^table\.data-table\b|\.card\b/` (`tests/e2e/tarjetas-geometria.spec.ts:199`) sí aceptaría un `.card-algo`, y un `grep -rhoE '\.card-[a-zA-Z0-9_-]+' design/ src/` devuelve **cero coincidencias**: hoy no existe ninguna clase así, la lista medida y la escrita coinciden elemento a elemento, y el CA dice exactamente eso. El follow-up es correcto y queda para el día que el sistema de diseño estrene su primera `.card-*`. | ✅ |
...
§Veredicto de la ronda 5, ni **ADR-034** ni **ADR-035** —que están **`aprobada`** desde el 2026-08-24, firmados por «humano (Alberto Fojo)», y esta reescritura no los toca ni los necesita: CA-15 se apoya en
...
### 🟢 GREEN — 2026-08-24, sdd-verificador (ronda 6, re-verificación)

**Los 21 CA en ✅.** El único finding abierto de la ronda 5 —**`F-VERIF-054-2`**, la
autocontradicción de CA-15— **está cerrado por la reescritura de la ronda 6**, y lo he
comprobado como corresponde: **leyendo la letra nueva contra la guardia** y **volviendo a
medir las seis propiedades** que ahora afirma. La reescritura no tocó código ni tests, y el
árbol lo confirma: los 16 `.txt` de evidencia volvieron **byte a byte idénticos** tras una
pasada completa y limpia de la e2e.

**Cambio de método respecto de la ronda 5, por decisión explícita del humano del 2026-08-24**:
esta ronda **mide con el módulo compartido `tests/e2e/geometria.ts`**, no con un arnés propio.
Conduje Playwright con la configuración del proyecto (`playwright.config.ts`, `webServer` =
`tests/e2e/server.mjs`, Postgres efímero en 54329, app en 3200) y una guardia **temporal** que
**importa las medidas del módulo** (`medirIntegridadDePalabra`, `medirPropiedadesComputadas`,
`medirCajas`, `medirDesbordePorElemento`, `medirAreaTactil`, `inyectarDefecto`, `ponerVentana`)
y el sembrado de `tests/e2e/spec054.ts`. Esa guardia **no afirma: imprime** —así el juicio es
mío y la medida es del proyecto— y **se borró al terminar**: `git status --porcelain` vacío.
**No he tocado ni una línea de código, spec ni test.**

**La contrapartida de este método, dicha de frente porque el humano la conoce y aun así lo
decidió**: reutilizar el módulo significa que **la guardia se verifica en parte consigo
misma**. Donde eso importa de verdad —donde el módulo **no puede** emitir el veredicto que el
CA pide— está escrito CA a CA en la matriz, y es el punto siguiente.

#### Lo que el módulo NO puede juzgar, y por eso no me fío de él más allá de lo que mide

**`medirAreaTactil` no puede cerrar CA-13.** Filtra con `m.ancho < suelo − tolerancia`
(`tests/e2e/geometria.ts:1207`, `tolerancia: TOLERANCIA_PX` en la 1237): **su suelo efectivo es
43**, y CA-13 exige 44 *«sin restarle ninguna holgura»*. Lo demostré con el propio módulo, en
`/cartera` @360:

| Con `padding-block: 10px` reinyectado (el defecto real de `F-VERIF-054-1`) | Veredicto |
|---|---|
| `medirAreaTactil` (M5, suelo efectivo 43) | `pequenos = 0` — **da por buena la situación que el CA prohíbe** |
| `medirCajas` + comparación contra 44 **a secas** (lo que hace `controlesBajoElSuelo`) | **8 controles a 43,00 exactos** |

Así que el ✅ de CA-13 **no lo emite M5**: lo emite el primitivo `medirCajas` del módulo con
una comparación sin holgura, que es exactamente lo que ADR-035 §1 manda y lo que la guardia
del proyecto hace desde la ronda 4. La otra reinyección, `DEFECTO_AREA_TACTIL`, sí la ve M5
(15 rasos a 16 px de alto en `/vigiladas` @360, sano 0, quitado 0) — porque 16 sí es menor que
43. **Las dos pruebas de eficacia siguen vivas y ninguna es decorativa.**

En **CA-15** el módulo sí puede juzgar: `medirIntegridadDePalabra` devuelve el `anchoLineaMax`
en crudo, así que la comparación contra la caja la hice **sin sumar tolerancia** y se cumple
igual. Lo que añade la tolerancia es la *guardia*, no el módulo.
...
| `npx playwright test` | ✅ **323/323** (4,7 min), pasada completa |
| `APP_BASE_URL=http://localhost:3200 npm run build` | ✅ exit 0 |
| `npm run version:check` | ✅ **exit 0**, `0.3.4 → 0.4.0`, sin abstención. **No la he tocado** |
| `_qa/` ajeno | ✅ mi pasada reescribió **228** capturas de otras 23 specs; restauradas con el `checkout` acotado. `git diff --name-only <base>...HEAD -- _qa` devuelve **sólo** `_qa/SPEC-054/` |
| Árbol al terminar | ✅ `git status --porcelain` vacío salvo este ledger |

#### CA-19, de propina: la reproducibilidad se ha vuelto a demostrar sola

Mi pasada completa de la e2e regeneró la evidencia y **los 16 `.txt` no aparecieron en el
diff**: byte a byte idénticos a los commiteados, en una máquina y un momento distintos de los
de la ronda 5. Los **10** `ancho-*.png` sí cambiaron y `capa-sobre-tarjeta-360.png` no —
exactamente lo que CA-19(b) predice y **no** afirma. **27 ficheros = 16 `.txt` + 11 `.png`.**
Los revertí todos: la evidencia commiteada es la que vale y yo no escribo en el árbol.

#### Los tres CA que la ronda 6 puso otra vez sobre la mesa

- **CA-15 → ✅.** La letra nueva es, cláusula a cláusula, lo que la guardia afirma, y las seis
  propiedades salen verdes medidas con el módulo. El detalle está en la matriz. La reescritura
  hace lo correcto: separa lo que es **de la caja** de lo que es **de la longitud de cada
  texto**, que es lo que hacía incumplible la redacción anterior.
- **CA-16 → ✅.** La (f) se prueba de un comando: `630  0`, cero líneas borradas, `TOLERANCIA_PX`
  intacto en la línea 79. Y la exhaustividad **la conté**: 16 ficheros de test en el diff
  acotado = 9 modificados (los 9 que el CA clasifica) + 7 nacidos aquí (los 7 que el CA
  declara, y ninguno existía en la base). **El hueco de la ronda 5 está cerrado.**
- **CA-21 → ✅.** El arquitecto tiene razón en que no cambia la aserción, pero no me fié: volví
  a leer el inventario y comprobé el follow-up. `grep -rhoE '\.card-[a-zA-Z0-9_-]+' design/ src/`
  → **cero coincidencias**. Hoy la lista medida y la escrita coinciden elemento a elemento.

#### La pregunta de ADR-035 §1: mi lectura

**Es material de `F-ADR-035-1`, no una salvedad que impida el ✅ de CA-15** — pero el argumento
de que «no aplica» **no me convence**, y conviene decirlo antes de que se dé por zanjado.

ADR-035 §1 **nombra este caso por su nombre**: *«un ancho contra los **34ch** de `.quote-*`»*
aparece **literalmente** en la lista de «medida contra umbral declarado», junto al 44 de M5 y
al 16/12 de ADR-034 §7. Y el ADR está **aprobado**. El argumento de que los dos lados salen
del motor es cierto de la *forma* de la comparación pero no de su *fondo*: el `max-width`
computado en px es la resolución de un número que **escribimos nosotros**, igual que el 44 es
un número nuestro aunque se compare contra un rect. Si eso bastara para reclasificarlo, el
mismo razonamiento reclasificaría el 44 y el ADR se quedaría sin contenido. **Mi lectura: sí
aplica, y la guardia de CA-15 tiene hoy una tolerancia donde un ADR aprobado dice que no debe
haberla.**

Ahora, **por qué eso no bloquea nada hoy, y lo he medido en vez de suponerlo**: la propiedad se
cumple **sin** la tolerancia, con **3,30 px** de margen en el caso más apretado (`.quote-stale`
de `/vigiladas`: 267,20 px de línea en una caja de 270,504). Retirar el `+ TOLERANCIA_PX` no
cambiaría ni un veredicto. Y CA-15, a diferencia de CA-13, **no pide «sin holgura» en su
letra**: pide que ninguna línea se extienda por encima de la caja, y no lo hace ni con
tolerancia ni sin ella.

**Lo que sí quiero dejar apuntado**: `F-ADR-035-1`, tal y como está redactado, **se queda
corto**. ADR-035 §2 («Consecuencia concreta sobre el módulo») baja la regla a `medirAreaTactil`
y a `medirSuelosTipograficos` — **los dos en el módulo**. Pero §1 declara **tres** casos, y el
tercero —los 34ch— **no vive en el módulo: vive en una guardia**. Un follow-up que sólo mire
`geometria.ts` dejará ese caso fuera sin que nadie se entere. Va para sdd-arquitecto.

#### Findings de redacción para sdd-arquitecto (ninguno bloquea el GREEN)

- **`F-SPEC-054-10` (nuevo) — la guardia de CA-15 afirma menos que su CA en la cláusula de la
  caja.** El CA dice *«`max-width: 34ch` computado, nunca `none`»*; la guardia sólo asegura
  `.not.toBe('none')` (`tarjetas-geometria.spec.ts:740-747`). Un futuro `max-width: 60ch`
  pasaría en verde siendo el defecto que SPEC-016 sufrió. **Hoy la propiedad se cumple —la
  medí: 270.504px = 34ch en Geist 12px, 244.8px = 34ch en Geist Mono 12px—**, así que no es un
  incumplimiento: es una regresión sin blindar. Misma clase que `F-SPEC-054-9`.
- **`F-ADR-035-1` (ampliar) — el tercer caso de ADR-035 §1 no está en su §2.** Ver arriba.
- **Ronda 6, párrafo final, dato caduco**: dice que ADR-034 y ADR-035 *«siguen en `borrador` y
  sin aprobar por el humano»*. Los dos están **`aprobada`** desde `d32a4dc` (frontmatter,
  `aprobada-por: humano (Alberto Fojo)`). No cambia ninguna decisión; es el texto el que va
  detrás del árbol. **No lo toco: no es mi columna.**

#### Preguntas abiertas para el humano

1. **`F-ADR-035-1` tiene ahora dos mitades, no una.** La conocida —`medirAreaTactil` con suelo
   efectivo 43— y la que este ledger añade: **la comparación de los 34ch en la guardia de
   CA-15**. Las dos salen del mismo §1 de un ADR ya aprobado. ¿Entran las dos en la spec propia,
   o sólo la del módulo? Yo diría las dos, y por el mismo motivo: **antes** de que la spec 2 de
   EPIC-007 escriba guardias nuevas.
2. **`F-SPEC-054-1` no es opcional para la spec 2**: los **12** controles de la nav, la marca, el
   pie y el enlace de feedback están medidos y muy por debajo de 44 (el menor, 36 × 26), y están
   escritos en `m5-fuera-de-alcance.txt` a propósito. La salida es **agrandarlos**; bajar el
   suelo sería `F-ADR-026-1` cumpliéndose por escrito.
3. **La evidencia visual queda en los bytes commiteados.** Mi pasada regeneró los 10
   `ancho-*.png` (relojes distintos, RN-16) y los **revertí**, porque el verificador no escribe
   en el árbol y CA-19(b) dice expresamente que esos píxeles no son evidencia. Si algún día se
   quiere una galería «recién hecha», eso es una decisión de la épica, no de esta spec.

**Estado de la spec**: la paso a **`hecho`** — es la transición terminal del verificador y es
mía. La versión ya está en **0.4.0** y `version:check` da exit 0 sobre el árbol limpio: **no la
he tocado**.
