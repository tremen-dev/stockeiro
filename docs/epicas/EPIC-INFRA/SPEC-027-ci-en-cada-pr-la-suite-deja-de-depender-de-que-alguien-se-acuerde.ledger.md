...
|---|---|---|---|---|
| CA-1 | `.github/workflows/ci.yml` (`on: pull_request/push`, ambos `branches: [main]`) | `tests/ci-workflow.test.ts` › *CA-1* (3 casos) **+ evidencia viva**: 6 ejecuciones disparadas por `pull_request` en la PR #31, la primera de ellas sobre el commit que introduce el propio workflow ([run 32086893473](https://github.com/tremen-dev/stockeiro/actions/runs/32086893473), success) | Estático reejecutado por mí: 3/3 verdes — `on` = {`pull_request`, `push`}, ambos con `branches: [main]`, ningún otro evento. **Vivo sobre el propio HEAD**: [run 32114768717](https://github.com/tremen-dev/stockeiro/actions/runs/32114768717) sobre `89e6f02`, evento `pull_request`, `Checks: success` + `E2E: success`. **Salvedad**: el disparador `push` a `main` NO se puede ejercitar antes de mezclar (el workflow todavía no existe en `main`), así que solo está probado estáticamente. Confirmar la ejecución del push justo después del merge. | ⚠️ |
| CA-2 | `.github/workflows/ci.yml` (steps `Typecheck`, `Lint`, `Unit tests`, `Build`, `End-to-end tests`) | `tests/ci-workflow.test.ts` › *CA-2* (2 casos) **+ evidencia viva**: en [run 32087238733](https://github.com/tremen-dev/stockeiro/actions/runs/32087238733) el rojo se lee en el nombre del step (`Typecheck: failure`, `Lint: failure`) sin abrir el log | Leído el YAML línea a línea: cinco steps de gate, exactamente uno por gate, y ningún `run` con dos comandos ni con `&&`. **Vivo**: la API de [run 32087238733](https://github.com/tremen-dev/stockeiro/actions/runs/32087238733) devuelve el estado POR NOMBRE DE STEP (`Typecheck=failure`, `Lint=failure`, `Unit tests=success`), que es literalmente el requisito del humano: el rojo se lee sin abrir el log. Estático 2/2. | ✅ |
| CA-3 | `.github/workflows/ci.yml` (`if: ${{ !cancelled() }}` en los tres gates de `Checks`; ausente en `End-to-end tests`) | `tests/ci-workflow.test.ts` › *CA-3* (2 casos) **+ prueba en rojo**: [run 32087238733](https://github.com/tremen-dev/stockeiro/actions/runs/32087238733) → `Typecheck: failure` · `Lint: failure` · **`Unit tests: success` (se ejecutó igual)** · job en rojo. Y la contrapartida en el mismo run: `Build: failure` → **`End-to-end tests: skipped`** (encadena a propósito). Commit `4197e23`, revertido en `9094a2c` | Rojo releído de la API, no del ledger: [run 32087238733](https://github.com/tremen-dev/stockeiro/actions/runs/32087238733) → `Typecheck=failure`, `Lint=failure` **y `Unit tests=success` (se ejecutó igual)**, job en rojo. La contrapartida en el mismo run: `Build=failure` → `End-to-end tests=skipped`, que es el encadenamiento deliberado. Estático 2/2 (los tres con `!cancelled()`, el e2e sin ella). | ✅ |
| CA-4 | `.github/workflows/ci.yml` (jobs `checks`/`Checks` y `e2e`/`E2E`, sin `needs`) | `tests/ci-workflow.test.ts` › *CA-4* (3 casos) **+ evidencia viva**: los dos jobs arrancan en el **mismo segundo** (`01:28:48Z` los dos, [run 32088353875](https://github.com/tremen-dev/stockeiro/actions/runs/32088353875)) y aparecen como dos checks separados en la PR; en [run 32087612615](https://github.com/tremen-dev/stockeiro/actions/runs/32087612615) uno falla y el otro pasa (`E2E: failure`, `Checks: success`), que es el aislamiento que el CA pide | Estático 3/3 (dos jobs, ninguno con `needs`, cada uno con sus gates). **Vivo sobre HEAD**: en [run 32114768717](https://github.com/tremen-dev/stockeiro/actions/runs/32114768717) `E2E` arranca a las `08:07:30Z` y `Checks` a las `08:07:32Z` — paralelos, dos entradas de check. Y el aislamiento real en [run 32087612615](https://github.com/tremen-dev/stockeiro/actions/runs/32087612615): `Checks: success` con `E2E: failure`. | ✅ |
| CA-5 | `.github/workflows/ci.yml` (`permissions: contents: read`; `env` de juguete en `E2E`; `timeout-minutes` 20/25; `concurrency` con `cancel-in-progress` condicionado) | `tests/ci-workflow.test.ts` › *CA-5* (6 casos, uno por punto: 5.1 sin `secrets.` / 5.1 variables de juguete / 5.2 permissions / 5.3 sin `db:migrate` / 5.4 timeouts / 5.5 concurrency). **5.4 demostrado en vivo, sin quererlo**: el `timeout-minutes: 25` cortó el job colgado de [run 32085219147](https://github.com/tremen-dev/stockeiro/actions/runs/32085219147) — el tope existe y muerde | Verificado punto por punto leyendo el YAML, no solo fiándome del test estático. 5.1 `grep -c "secrets\." .github/workflows/ci.yml` = **0**, y las cuatro variables son de juguete en claro. 5.2 `permissions: {contents: read}` a nivel de workflow. 5.3 ningún `run` contiene `db:migrate` ni `drizzle-kit migrate`. 5.4 `timeout-minutes` 20 en `Checks` y 25 en `E2E`, más 5 en el step del navegador. 5.5 `concurrency.group` lleva `github.ref` y `cancel-in-progress: ${{ github.event_name == 'pull_request' }}`. **Reproducido por mí el hallazgo del plan**: `gh api .../branches/main/protection` y `.../rulesets` responden los dos `403 Upgrade to GitHub Pro` — la CI informa y no impide, y el runbook lo dice. Estático 6/6. | ✅ |
| CA-6 | `.nvmrc` (`24`) + `node-version-file: .nvmrc` en los dos `Set up Node` | `tests/ci-workflow.test.ts` › *CA-6* (2 casos) **+ el runner lo imprime**: `Found in cache @ /opt/hostedtoolcache/node/24.19.0/x64` y `node: v24.19.0`, con `node-version-file: .nvmrc` en el log. Suite completa ejecutada además en Node 24 en local — ver §Mediciones | `.nvmrc` = `24`; los dos `Set up Node` usan `node-version-file: .nvmrc` y **no hay ningún `node-version:` en el YAML**. Log del runner: `node-version-file: .nvmrc`, `Found in cache @ /opt/hostedtoolcache/node/24.19.0/x64`, `node: v24.19.0`. **Y la suite entera reejecutada por mí en Node 24.19.0** (binario portable antepuesto al PATH; la máquina sigue en 22.19.0): `npm run typecheck` exit 0 · `npm run lint` exit 0 · `npm run test` **308/308 en 32 ficheros, 118,94 s** · `npm run build` exit 0 · `npm run test:e2e -- --forbid-only …` **27/27, 1,5 min**. Node 24 no rompe nada. Estático 2/2. | ✅ |
| CA-7 | `package.json` › `scripts.lint` = `eslint . --max-warnings=0`, `scripts["test:e2e"]` = `playwright test`; todo gate del YAML invoca `npm run <script>` | `tests/ci-workflow.test.ts` › *CA-7* (3 casos: los scripts existen; cada gate invoca el suyo; las banderas van tras `--`). En el log de CI se lee el comando entero: `> playwright test --forbid-only --trace=retain-on-failure --reporter=list,html` | Estático 3/3. Y comprobado a mano lo que más importaba: **ninguna bandera del YAML cambia lo que se ejecuta respecto de local**. `--max-warnings=0` vive DENTRO de `scripts.lint` (`eslint . --max-warnings=0`), no en el workflow, así que `npm run lint` significa lo mismo en la máquina del humano y en el runner — ejecutado aquí, exit 0 con 0 warnings. Las tres banderas de Playwright van tras `--`, son de rigor o diagnóstico, y el YAML explica cada una. El log de CI enseña el comando entero: `> playwright test --forbid-only --trace=retain-on-failure --reporter=list,html`. | ✅ |
| CA-8 | `.github/workflows/ci.yml` (`cache: npm` en setup-node; `actions/cache` de `~/.cache/ms-playwright` con clave de `package-lock.json`; sin caché de `node_modules` ni `.next/cache`, con el motivo escrito en el YAML) | `tests/ci-workflow.test.ts` › *CA-8* (4 casos) **+ ahorro medido** — ver §Ahorro de caché. Frío→caliente: `Install Playwright browser` **14 s → 1 s**; `Cache hit` de 261 MB (navegador) y 212 MB (npm) en el log | Estático 4/4. **Vivo** en el log de [run 32114303596](https://github.com/tremen-dev/stockeiro/actions/runs/32114303596): `Cache hit for: node-cache-Linux-x64-npm-9d37e1f0…` y `Cache hit for: playwright-Linux-9d37e1f0…`, con `Install Playwright browser` en 1 s. Ningún `actions/cache` ni `cache-dependency-path` menciona `node_modules` ni `.next`, y el porqué está escrito en los comentarios del propio YAML (`node_modules`, `embedded-postgres`, `.next/cache`). | ✅ |
| CA-9 | `.github/workflows/ci.yml` › step `Upload e2e diagnostics` (`if: failure()`, `playwright-report/` + `test-results/` + `_qa/`, `retention-days: 7`) | Sin test estático a propósito: lo que se afirma es que el artefacto *existe y abre*. **Prueba en rojo**: [run 32087968719](https://github.com/tremen-dev/stockeiro/actions/runs/32087968719) → artefacto `e2e-diagnostics` de **9,8 MB**, descargado y abierto — ver §Artefacto. **Y en verde no sube nada**: [run 32088353875](https://github.com/tremen-dev/stockeiro/actions/runs/32088353875) tiene **0 artefactos**. Commit `e8036d4`, revertido en `e2f7b2a` | **Artefacto descargado y abierto por mí**, no leído del ledger: `gh run download 32087968719` → `e2e-diagnostics`, **9 804 365 B**, con `playwright-report/` (+ `data/`, `trace/`), `test-results/vigiladas-SPEC-003-…-chromium/trace.zip` de **1 125 825 B y 60 entradas** (`test.trace`, `0-trace.trace`, `0-trace.network`, `0-trace.stacks` y el screencast en `resources/*.jpeg`) y `_qa/` con **50 PNG**. No es un fichero con nombre bonito: la traza tiene dentro lo que dice tener. Retención por API: creado `2026-08-18T01:24:10Z`, expira `2026-08-25T01:24:09Z` = **7 días exactos**. **Y en verde no sube nada**: `total_count: 0` en los runs 32088353875, 32114303596 y 32114768717, con `Upload e2e diagnostics=skipped`. | ✅ |
| CA-10 | `.github/workflows/ci.yml` › `npm run test:e2e -- --forbid-only …` | **Prueba en rojo**: [run 32087612615](https://github.com/tremen-dev/stockeiro/actions/runs/32087612615) → `End-to-end tests: failure` con el mensaje literal `Error: item focused with '.only' is not allowed due to the '--forbid-only' CLI flag: "vigiladas.spec.ts SPEC-003: …"`. Sin la bandera habría pasado en verde con **1 test de 27**. Commit `8f4185f`, revertido | Log de [run 32087612615](https://github.com/tremen-dev/stockeiro/actions/runs/32087612615) releído: `Error: item focused with '.only' is not allowed due to the '--forbid-only' CLI flag: "vigiladas.spec.ts SPEC-003: …"`, con `End-to-end tests=failure` y `Checks=success`. La bandera está en el `run` del YAML y la he ejercitado en local sobre la rama limpia: 27/27 con `--forbid-only`. Sin ella ese run habría dado verde con **1 test de 27**. | ✅ |
| CA-11 | `tests/schema-source.test.ts` › bloque CA-6, **reimplementado tras la enmienda del 2026-08-18** (`231b2c6`): canario sobre **sonda sembrada y rebobinada** al primer apunte del journal; la **forma de la ruta de sonda vive en un solo sitio** (`withProbe`), ni guardia ni canario nombran su `--out` | `tests/schema-source.test.ts` › *CA-6* › «la guardia sabe detectar: sobre una sonda rebobinada reproduce las migraciones que faltan». **Verde + las DOS roturas exigidas, ambas revertidas** — ver §CA-11 reimplementado | **Los tres experimentos reproducidos por mí en este worktree** (edición temporal de una línea + `git checkout --`; árbol limpio al terminar, `git status --porcelain` vacío). **(0) Verde de partida**: guardia 2 550 ms, canario 2 465 ms, los dos ✓. **(1) Invocación rota** (`--schema ./src/db/NO-EXISTE-VERIF.ts`): **los dos revientan** con `Command failed: npx drizzle-kit generate --dialect postgresql --schema ./src/db/NO-EXISTE-VERIF.ts --out …` — señala la invocación, no la deriva. **(2) La que justifica la enmienda** (`const outArg = relPath` → `= dir`, el sitio único): **guardia ✓ VERDE EN FALSO (2 121 ms) y canario × ROJO (2 014 ms)** con el mensaje «La guardia de esquema NO PUDO EJECUTARSE…» y su segunda lectura. **(3) Contrafactual, porque la enmienda entera se apoya en él**: con ese mismo `outArg` absoluto y el canario devuelto a **sonda vacía** (la forma de `6c2a625`), **los dos vuelven a verde** — la premisa del implementador se sostiene, medida por mí, no es relato. Coste: **2 465 ms** en local (Node 24) y **655 ms** en CI ([run 32114303596](https://github.com/tremen-dev/stockeiro/actions/runs/32114303596)), muy por debajo del techo de 10 s. | ✅ |
| CA-12 | `tests/schema-source.test.ts`: la inspección `/error\|ENOENT/i` sigue retirada (**no se revirtió**) y el comentario recoge las **tres rondas** medidas, incluido el mecanismo real: `drizzle-kit` concatena el cwd delante de la ruta absoluta al buscar `meta/NNNN_snapshot.json` | Lectura del fichero: **no queda ninguna inspección de la salida del proceso ni ninguna aserción sobre la forma de la ruta**, ni a favor ni en contra (descartado por el humano el 2026-08-18). El comportamiento lo verifica CA-11 | Lectura del fichero. 12.1: la inspección `/error\|ENOENT/i` **no está** — el diff la borra y no vuelve; los únicos restos de `ENOENT`/`stdout` viven en el comentario, que es lo que el CA pide. 12.2: el comentario de `withProbe` recoge **las cinco filas medidas** de las tres rondas y el mecanismo real (drizzle-kit concatena el cwd delante de la ruta absoluta al buscar `meta/NNNN_snapshot.json`). 12.3: de los **16 `expect(`** del fichero **ninguno** habla de la forma de la ruta, ni a favor ni en contra (los `toMatch` que hay son los FK de CA-2 de SPEC-026). La decisión del humano del 2026-08-18 se respeta. | ✅ |
| CA-13 | Sin cambios de expectativas en ningún test existente; único fichero de test existente tocado: `tests/schema-source.test.ts` (+ `tests/position.test.ts`, un import sin usar, autorizado en el gate) | Suite completa verde en Node 22 y Node 24 en local (308/308 en 32 ficheros) **y en CI** (32/32 ficheros, 27/27 e2e). **Coste aislado del canario SEMBRADO medido** — ver §CA-11 reimplementado. Tiempos y minutos facturados reales — ver §Tiempos reales de CI | **308/308 en 32 ficheros, 118,94 s** en Node 24 local — el techo era 145 s +25 % = 181 s y no se acerca; y 32/32 ficheros + 27/27 e2e en CI sobre HEAD ([run 32114768717](https://github.com/tremen-dev/stockeiro/actions/runs/32114768717)). Los 26 tests nuevos cuadran: 25 casos en `tests/ci-workflow.test.ts` + 1 canario. **Ninguna expectativa ajena cambia**: en `git diff c1d6ae6 HEAD -- tests/schema-source.test.ts` la única línea con `expect(` que aparece es un `expect(` reindentado, de contenido idéntico. `git diff c1d6ae6 HEAD -- tests/` toca **4** ficheros y no 3: el cuarto es `tests/e2e/server.mjs`, y su cambio es **una línea de comentario** del renombrado ADR-018→ADR-019 (`e930f1c`), igual que el de `src/db/test-db.ts`. No es desviación. | ✅ |

## Veredicto del verificador
<!-- GREEN/RED + fecha + resumen. Lo escribe SOLO sdd-verificador. -->

**GREEN — 2026-08-18, sdd-verificador.** 12 CA en ✅ y **CA-1 en ⚠️ justificada y aceptada**
(el disparador `push` a `main` no se puede ejercitar antes de mezclar: el workflow todavía no
existe en `main`; queda probado estáticamente y hay que mirar el run del push justo después del
merge). Ningún finding bloqueante.

### Lo que he ejecutado yo, en esta rama y en este worktree

Todo en **Node 24.19.0** (binario portable antepuesto al `PATH`; la máquina sigue en 22.19.0),
que es la versión que fija `.nvmrc` y la que corre Vercel:

| Gate | Resultado |
|---|---|
| `npm run typecheck` | exit 0 |
| `npm run lint` (`eslint . --max-warnings=0`) | exit 0, **0 errores y 0 warnings** |
| `npm run test` | **308/308 en 32 ficheros, 118,94 s** |
| `npm run build` (variables de juguete del YAML) | exit 0 |
| `npm run test:e2e -- --forbid-only --trace=retain-on-failure --reporter=list` | **27/27, 1,5 min** |

Y sobre el propio HEAD (`89e6f02`), la CI real: [run
32114768717](https://github.com/tremen-dev/stockeiro/actions/runs/32114768717) —
`Checks: success` (08:07:32Z→08:12:22Z) y `E2E: success` (08:07:30Z→08:09:22Z), **0 artefactos**,
`Upload e2e diagnostics=skipped`. Es la ejecución que respalda CA-1…CA-8 y CA-13 sobre el commit
que se va a mezclar, no sobre uno anterior.

### La enmienda de CA-11, reproducida por mí y no leída del ledger

Es lo más nuevo de la spec y lo que había pasado por tres veredictos contradictorios, así que lo
he medido yo, con ediciones temporales de una línea revertidas con `git checkout --` (árbol
limpio al terminar: `git status --porcelain` vacío):

| Experimento | Guardia | Canario | Lectura |
|---|---|---|---|
| **(0)** tal cual está en la rama | ✓ 2 550 ms | ✓ 2 465 ms | verde de partida |
| **(1)** `--schema` a un fichero inexistente | ✗ `Command failed: npx drizzle-kit generate …` | ✗ ídem | los dos revientan; señalan la **invocación**, no la deriva |
| **(2)** `const outArg = relPath` → `= dir` (sonda absoluta, sitio único) | **✓ VERDE EN FALSO** 2 121 ms | **✗ ROJO** 2 014 ms, «La guardia de esquema NO PUDO EJECUTARSE…» | **el canario es lo único que se entera** — CA-11 cerrado |
| **(3)** contrafactual: (2) + canario devuelto a **sonda vacía** (forma de `6c2a625`) | ✓ | ✓ | **los dos en verde**: la premisa de la enmienda se sostiene, medida |

El experimento (3) importaba tanto como el (2): la enmienda entera se apoya en que el canario
viejo **no** cazaba este fallo. Lo he comprobado, y es cierto.

### Lo que he apretado además de los CA

- **El workflow no puede hacer daño (CA-5)**, verificado leyendo el YAML y no solo el test:
  cero `secrets.`, `permissions: contents: read`, ningún `run` con `db:migrate` ni
  `drizzle-kit migrate`, `timeout-minutes` en los dos jobs (20 y 25) más 5 en el step del
  navegador, y `cancel-in-progress` condicionado a `pull_request`. He reproducido también los dos
  `403 Upgrade to GitHub Pro` (protección de rama y *rulesets*): el hallazgo del plan es real y
  el runbook lo dice sin ambigüedad, como pedía la decisión humana.
- **CA-7, el punto fino**: `--max-warnings=0` está dentro de `scripts.lint`, no en el YAML, así
  que `npm run lint` es el mismo comando en local y en CI. Ninguna bandera del workflow cambia
  qué se ejecuta.
- **Atribución de evidencia**: los seis runs viejos corrieron con el canario anterior y **no**
  valen para CA-11; el aviso del implementador es correcto y lo he respetado. CA-11 se apoya en
  mis mediciones locales más los runs 32114303596 y 32114768717.
- **Retirada de `--with-deps`**: **no es desviación.** Venía de §*Forma del workflow*, marcada en
  la spec como *«guía para el implementador, no contrato»*, y ningún CA la menciona. Riesgo
  residual valorado: si a `ubuntu-latest` le faltara una librería, Chromium no arrancaría y el
  e2e lo diría a gritos — no es un modo de fallo silencioso. Empíricamente, Chromium arranca y
  pasa 27/27 en cinco ejecuciones distintas del runner.
- **Renombrado ADR-018/ADR-019** (`e930f1c`): coherente en id, título, nombre de fichero y
  tablero. Las referencias de código (`src/db/test-db.ts`, `tests/e2e/server.mjs`,
  `tests/schema-source.test.ts`) apuntan a **ADR-019** y las de CI a **ADR-018**. Sin referencias
  rotas. Una salvedad documental, abajo.

### Residuales antes de mezclar (ninguno bloquea)

1. **CA-1, mitad `push`**: mirar la ejecución del push a `main` inmediatamente después del merge.
   Es la única pieza de un CA que no se puede probar antes.
2. **La spec quedó desactualizada por su propio renombrado.** `SPEC-027` §Transversal sigue
   diciendo *«Contexto: **ADR-018 — El esquema de test es el de producción** (SPEC-026)»* y
   *«Colisión de ids detectada, y **no la arregla esta spec**: hay dos ADR con id `ADR-018`»*, y
   la nota 8 del gate repite lo mismo. La colisión **sí** la arregló esta rama (`e930f1c`), así
   que el documento contradice al árbol y manda a quien lo lea al ADR equivocado. No es un CA y
   no cambia una línea de código: → **documentalista**, o corrección al vuelo del arquitecto.
3. **F-SPEC-027-1** (la CI informa y no impide) y **F-SPEC-027-2** (guardias de migración,
   bloqueante de SPEC-028) siguen abiertos y bien declarados. **F-SPEC-027-5**
   (`actions/checkout@v4` y `setup-node@v4` deprecadas) es deuda con fecha, no urgencia.
4. **Lo que este gate NO cubre y conviene no olvidar**: una matriz de Node/SO, `npm audit` y la
   cobertura. Están fuera de alcance a propósito; solo lo dejo escrito para que el verde de
   `CI / Checks` no se lea como más de lo que es.
...
**Confirmado por el verificador**: no hay nada que capturar en `_qa/SPEC-027/`. Lo más parecido a
una captura aquí es el estado por step que devuelve la API de Actions, y está pegado literalmente
en la matriz (CA-2, CA-3, CA-4, CA-9). Al ejecutar el e2e en local se reescribieron las capturas
de `_qa/` de otras specs; **restauradas con `git checkout -- _qa`**, árbol limpio
(`git status --porcelain` vacío antes de escribir este ledger).
