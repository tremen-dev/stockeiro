...
### 🔴 RED — 2026-08-25 (4.ª ronda), sdd-verificador

**`F-SPEC-052-15` está CERRADO de verdad: la mitad de `DATABASE_URL` de esa frase es hoy
correcta, comprobada literal a literal y corrida a corrida. Pero al medir la mitad que el
encargo me pidió confirmar —la de `APP_BASE_URL`, que esta ronda dejó intacta *porque su
ruta sí es estable*— resulta que NO es estable: seis corridas, cuatro rutas distintas, y
`/_not-found` solo una de seis. La frase gemela tiene exactamente el mismo defecto que
`F-SPEC-052-15`, en el mismo párrafo y bajo la misma palabra «Medido». Finding único:
`F-SPEC-052-17`.**

**El error de origen es MÍO, y conviene que quien lea esto lo sepa antes de repartir
culpas.** En la 3.ª ronda escribí, en este mismo ledger, *«la mitad de `APP_BASE_URL` sí es
estable … Esa mitad se queda como está»*, y lo escribí sobre **tres** observaciones que
salieron `/_not-found` por suerte. El implementador hizo **exactamente** lo que el gate le
dijo, y lo dejó anotado como tal. Lo que devuelvo no es un incumplimiento de la instrucción
recibida: es que la instrucción estaba mal, y el artefacto que quedaría en `main` afirma
algo falso.

**Gates automáticos, ejecutados por mí sobre `de580a9` con el árbol limpio:** `npm test`
→ **1829/1829, 114/114 ficheros**, exit 0, 181 s, **a la primera y sin un solo *timeout***
(el aviso sobre los cuatro ficheros de Postgres efímero no se ha reproducido: era
contención de máquina, como se sospechaba); `npm run typecheck` exit 0; `npm run lint`
exit 0; `npm run version:check` con el árbol **limpio** (`git status --porcelain` vacío) →
*«El diff no toca codigo de aplicacion: no hay nada que subir»*, exit 0. `origin/main` en
**`497eccf`**: la base no se ha movido.

**Alcance de la ronda, comprobado y no creído.** `git diff --stat e82a347..de580a9` → **tres
ficheros**: `docs/despliegue.md` (**un solo hunk**, líneas 275-283, dentro de §3.2 —que va de
la 252 a la 329—), el ledger y el frontmatter de la spec. **§0 (líneas 89-177) no aparece en
el diff.** Ni un fichero de `tests/`, ni `.env.example`, ni nada bajo `src/` — y el diff
completo de la rama contra `497eccf` tampoco toca `src/`. Los **dos rótulos literales** de
CA-7 siguen palabra por palabra en `docs/despliegue.md:274` y `:297`, y son los que clavan
`tests/entornos-de-despliegue.test.ts:553-554`. Las columnas **Verif.** y **Estado**:
comparadas fila a fila (18 filas) entre el commit del gate y el del implementador en las
**tres** últimas rondas con un script propio → **sin un solo cambio** en ninguna de las tres.
La declaración del implementador es cierta.

**Lo que he medido, corrida a corrida, con `.next` borrado antes de cada build**
(`npx next build`, entorno construido con `env -u`, `.env.local` solo lleva
`VERCEL_OIDC_TOKEN` y no interfiere). Diez corridas nuevas, ninguna heredada:

| # | Entorno | Punto de muerte | Ruta que sale en el log |
|---|---|---|---|
| 1 | sin `DATABASE_URL` | *Collecting page data* | `/admin` |
| 2 | sin `DATABASE_URL` | *Collecting page data* | `/cuenta` |
| 3 | sin `DATABASE_URL` | *Collecting page data* | `/api/cron/refresh` (sin línea `Failed to collect configuration for …`) |
| 4 | sin `DATABASE_URL` | *Collecting page data* | `/api/cron/refresh` (ídem) |
| 5 | **sin `APP_BASE_URL`** | *Collecting page data* | **`/ayuda`** |
| 6 | **sin `APP_BASE_URL`** | *Collecting page data* | **`/forgot-password`** |
| 7 | **sin `APP_BASE_URL`** | *Collecting page data* | **`/_not-found`** |
| 8 | **sin `APP_BASE_URL`** | *Collecting page data* | **`/admin`** |
| 9 | **sin `APP_BASE_URL`** | *Collecting page data* | **`/ayuda`** |
| 10 | **sin `APP_BASE_URL`** | *Collecting page data* | **`/admin`** |

Las diez dicen `Collecting page data using 15 workers ...` y las diez salen con **exit 1**.
El *«muere en el mismo punto»* del documento es **cierto**; lo que no es cierto es la ruta,
**en ninguna de las dos mitades**.

**Lo que sí está bien en la frase nueva, y lo digo con el mismo detalle con que devuelvo lo
otro.**

- **El mensaje literal coincide carácter a carácter.** Extraído del documento y de
  `src/db/client.ts:19` y comparado con `od -c`: los 59 bytes son idénticos, acento de
  `Configúrala` incluido (`303 272`). Y es literalmente la línea que imprime el build en mis
  cuatro corridas sin `DATABASE_URL`. Va **completo**, no abreviado: acierto — así se
  reconoce en un log.
- **Las cuatro rutas enumeradas están todas observadas y todas existen.** `/admin`,
  `/cartera/importar`, `/api/cron/refresh` y `/api/auth/[...nextauth]` tienen su
  `page.tsx`/`route.ts` en `src/app/`, y las cuatro están escritas en este ledger por el
  gate (corridas C, C2, C4 de la 3.ª ronda y la de la 2.ª). **Ninguna inventada, ninguna
  completada «por simetría».** La frase dice *«se han observado»* —existencial, no
  exhaustivo—, y por eso mi corrida 2 (`/cuenta`, una quinta ruta) **no la falsifica**.

**Las tres omisiones declaradas: las tres son correctas, y ninguna deja al lector peor.**

1. **El número de *workers*.** Bien omitido. Aquí son 15, en el log real del PR #58 es 1:
   escribirlo sería exactamente la sobre-precisión que `F-SPEC-052-15` castiga. El
   documento se queda con el mecanismo (*«las páginas se recogen en paralelo»*), que es lo
   que el lector necesita y lo único que no depende de la máquina.
2. **La corrida ambigua (la que yo anoté como «solo capturé el mensaje»).** Bien omitida —y
   ahora puedo cerrar la ambigüedad que yo dejé abierta: mis corridas 3 y 4 reproducen ese
   log y explican la asimetría. Cuando quien revienta es una *route handler*, Next **no**
   imprime `Failed to collect configuration for …`; imprime el error pelado y luego
   `Failed to collect page data for /api/cron/refresh`. Es decir: sí había ruta, en otra
   línea. Omitir sin afirmar nada era la decisión correcta con la información que tenía.
3. **No uniformar con `APP_BASE_URL`.** Correcta **como obediencia** —era mi instrucción
   expresa— e **incorrecta como hecho**, y eso es `F-SPEC-052-17`. La asimetría que el
   párrafo enseña («la de `DATABASE_URL` varía, la de `APP_BASE_URL` no») no solo es falsa:
   está **invertida en el mecanismo**. `appBaseUrl()` se evalúa en `metadataBase` del layout
   raíz (`src/app/layout.tsx:67`), que **todas** las rutas importan, mientras que el cliente
   de BD solo lo importan algunas. La carrera de `APP_BASE_URL` es **más ancha**. Un lector
   que se crea el contraste sale con un modelo mental peor que si no hubiera leído nada.

**Por qué es RED y no una salvedad.** Este bloque se presenta a sí mismo como texto de
diagnóstico —*«decide dónde vas a ver el fallo»*— y esta es la **quinta** afirmación de la
misma familia en la entrega (`-10`, `-11`, `-12`, `-15` y ahora `-17`). Cinco de seis
lectores que borren `.next` y construyan sin `APP_BASE_URL` verán una ruta que el runbook no
nombra, en el párrafo cuya única razón de existir es que la sepan reconocer. El arreglo es
**una frase** y no toca ningún test: `F-SPEC-052-17` lo trae escrito.

### 🔴 RED — 2026-08-25 (3.ª ronda), sdd-verificador

**`F-SPEC-052-12` está CERRADO de verdad: he rehecho las cinco corridas y una más, y
todo lo que §3.2 afirma sobre `AUTH_SECRET`, `AUTH_TRUST_HOST` y `ALLOW_MIGRATE` es
cierto y está bien separado. La corrección no se ha llevado por delante ningún criterio
ya verde. Pero en el mismo párrafo nuevo hay una cuarta afirmación de la misma familia
—una ruta concreta, presentada como «Medido», que no se reproduce—, y por eso CA-7 sigue
sin cerrar. Finding único: `F-SPEC-052-15`.**

**Gates automáticos, ejecutados por mí sobre `1431630` con el árbol limpio:** `npm test`
→ **1829/1829, 114/114 ficheros**; `npm run typecheck` exit 0; `npm run lint` exit 0;
`npm run version:check` con el árbol limpio → *«El diff no toca codigo de aplicacion: no
hay nada que subir»*, exit 0. `origin/main` sigue en **`497eccf`**: la base no se ha
movido. <sub>Honestidad sobre el harness: la **primera** pasada de la suite dio 1826/1829
con tres ficheros en *timeout* de 20 s (`account-deletion`, `spec044-edicion-zonas`,
`zone-status`) porque tenía dos cosas peleándose por la máquina. Los tres, re-ejecutados
en aislamiento, **51/51 verde**; y la pasada final, sola, **1829/1829**. El diff no toca
ni una línea de código, así que no había regresión posible que medir.</sub>

**Alcance de la ronda, comprobado y no creído.** `git diff --name-only c9d8405..HEAD` →
tres ficheros: `docs/despliegue.md`, el ledger y el frontmatter de la spec. **Ni un
fichero de `tests/`, ni `.env.example`, ni nada bajo `src/`.** Los tres *hunks* de
`docs/despliegue.md` caen todos en o después de la línea 267: **§0 no se ha tocado** —la
tabla sigue con las cuatro claves en `Preview + Production` y CA-8 sigue exigiéndolo con
razón—. Las celdas **Verif.** y **Estado** de la matriz llegaron **intactas**: el
implementador solo tocó la columna *Implementado* de la fila de CA-7 (el diff del ledger
tiene una sola línea borrada, y es esa).

**Lo que he medido, corrida a corrida, con `.next` borrado antes de cada build.** Ninguna
de estas líneas viene del informe del implementador:

| Corrida | Entorno | Resultado medido por el gate |
|---|---|---|
| **A** — `next build` | `DATABASE_URL` + `APP_BASE_URL` **y nada más** | **verde, exit 0**, 21/21 páginas, listado de rutas completo, `.next/BUILD_ID` escrito |
| **B** — `next build` | sin `APP_BASE_URL` | **rojo, exit 1** — *Collecting page data* → `Failed to collect configuration for /_not-found`, causa `APP_BASE_URL no definida (ver .env.example)`, traza terminando en `L.layout` |
| **C** — `next build` | sin `DATABASE_URL` | **rojo, exit 1** — `DATABASE_URL no definida. Configúrala (ver .env.example).` recogiendo **`/admin`** |
| **C2/C3/C4** — `next build` | sin `DATABASE_URL`, repetido tres veces | **rojo** siempre, mismo mensaje, rutas **`/cartera/importar`**, (no capturada) y **`/api/auth/[...nextauth]`** |
| **D** — `next start` sobre A | sin `AUTH_SECRET` ni `AUTH_TRUST_HOST` | `/login` **200**, `/legal` **200**, `/dashboard` **307**, `POST /api/auth/callback/credentials` **500** con *«There was a problem with the server configuration»*; log: `UntrustedHost: Host must be trusted` |
| **E** — `next start` sobre A | **con** `AUTH_TRUST_HOST=true`, **sin** `AUTH_SECRET` | mismas tres páginas, mismo `POST` **500**; log: `MissingSecret: Please define a 'secret'` |
| **F** — `next start` sobre A | **con** `AUTH_SECRET`, **sin** `AUTH_TRUST_HOST` | mismas tres páginas, mismo `POST` **500**; log: `UntrustedHost` |
| **G** — cadena del `buildCommand` | `VERCEL_ENV=preview`, sin `ALLOW_MIGRATE` | `guard-migrate.mjs` **exit 1**; el `&&` corta y el eslabón siguiente **no llega a ejecutarse** |

**Qué prueba cada una, y por qué basta.** **A** no solo sostiene el tercer cajón: como el
entorno estaba reducido a dos claves, prueba el *«y solo esas dos»* frente a **todas** las
filas de §0, no solo frente a las cuatro del `env` de CI. **B** y **C** son el control
positivo que exige el encargo: el harness detecta fallos, así que el verde de **A** no es
caché. **E** y **F** existen porque `UntrustedHost` se comprueba antes que el secreto y lo
tapa: separadas, sostienen **cada mitad** de la frase de §3.2 por su cuenta, y esa frase
—*«`UntrustedHost` cuando falta `AUTH_TRUST_HOST` y `MissingSecret` cuando falta
`AUTH_SECRET`»*— es **exacta**. **G** cierra el párrafo de `ALLOW_MIGRATE`: el
`buildCommand` de `vercel.json` es
`node scripts/guard-migrate.mjs && npm run db:migrate && npm run build`, y con
`VERCEL_ENV=preview` sin permiso la guardia sale con **1** — sacarla del primer cajón era
correcto, y el mecanismo que ahora describe es el real.

**La nota de la cota superior también es cierta.** El único job con un `run` de
`npm run build` es `E2E`; su bloque `env` declara **cuatro** claves y **A** demuestra que
el build solo exige **dos**: el conjunto derivado es un **superconjunto** del que rompe el
build, así que la guardia de CA-8 **sobreaproxima** — y una sobreaproximación es sana
mientras el documento no la venda como igualdad, que es justo lo que la nota nueva
corrige. **No contradice a CA-8**: CA-8 exige presencia en §0 como `Preview + Production`,
y las cuatro la tienen.

**Y también he comprobado lo que NO se ha movido.** Los dos rótulos que CA-7 clava —
*«Obligatorias en Preview porque el build las lee»* y *«solo hacen la preview más útil»*—
siguen **palabra por palabra** en §3.2 (líneas 274 y 292) y las dos frases prohibidas
siguen sin aparecer en todo el fichero. `.env.example`, la tabla de §0, el vocabulario
cerrado y sus tres valores, `src/` y `tests/`: intactos.

**Por qué aun así es RED.** §3.2 dice, con `Medido` por delante: *«sin `DATABASE_URL`,
recogiendo `/api/auth/[...nextauth]`»*. Lo he medido **cuatro veces sobre este mismo árbol
y me han salido cuatro rutas distintas** — y una quinta, `/api/cron/refresh`, está escrita
por mí en este ledger desde la 2.ª ronda. Es la cuarta afirmación de la misma familia en
la misma entrega: una frase que suena precisa, que se midió **una vez**, y que el runbook
presenta como *el* síntoma. La mitad de `APP_BASE_URL` sí es estable (dos corridas mías
hoy, la de la 2.ª ronda y el log real del PR #58, todas `/_not-found`); la de
`DATABASE_URL` no lo es, y no puede serlo: `next build` recoge las páginas **en paralelo**
y revienta la primera que evalúe el cliente de BD. `F-SPEC-052-15`, con el texto de
recambio ya escrito.
...
- **F-SPEC-052-17** 🔴 **ABIERTO — finding único del gate del 2026-08-25 (4.ª ronda).
  La frase GEMELA de `F-SPEC-052-15`, la de `APP_BASE_URL`, tiene el mismo defecto: da
  `/_not-found` como firma medida y sale 1 de cada 6 veces. Y el origen del error es del
  gate, no del implementador.**

  `docs/despliegue.md` §3.2, primer cajón, última frase — **la mitad que esta ronda dejó
  intacta a propósito**:
  ...
  **La medición.** Seis `next build` sin `APP_BASE_URL` (con `DATABASE_URL` presente),
  `.next` borrado antes de cada uno, mismo árbol, mismo commit `de580a9`, misma máquina:

  | Corrida | Ruta que aparece en `Failed to collect page data for …` |
  |---|---|
  | 1 | `/ayuda` |
  | 2 | `/forgot-password` |
  | 3 | **`/_not-found`** |
  ...
  **Cuatro rutas distintas; `/_not-found` una de seis.** Las seis mueren en *Collecting page
  data* con `Collecting page data using 15 workers ...` y `exit 1`, y las seis dan como causa
  `APP_BASE_URL no definida (ver .env.example): sin ella no hay enlaces válidos.`
  (`src/lib/config/app-url.ts:87`). En varias corridas el log imprime **dos o más**
  `Failed to collect configuration for …` seguidas, de rutas distintas: la carrera se ve a
  simple vista.

  **Y tenía que salir así.** `appBaseUrl()` se evalúa en `metadataBase` del layout raíz
  (`src/app/layout.tsx:67`), que importan **todas** las rutas; el cliente de BD
  (`src/db/client.ts:19`) solo lo importan algunas. Es decir: la carrera de `APP_BASE_URL`
  es **más ancha** que la de `DATABASE_URL`, no más estrecha. El párrafo, tal como está,
  enseña la asimetría **al revés**.

  **Este error lo introdujo el gate, no el implementador.** En la 3.ª ronda escribí en este
  ledger, dentro del propio `F-SPEC-052-15`: *«Nótese lo que NO está mal … la mitad de
  `APP_BASE_URL` **sí** es estable —`/_not-found` en mis dos corridas de hoy, en la de la
  2.ª ronda y en el log real de Vercel del PR #58— … Esa mitad se queda como está»*. Tres
  observaciones locales, las tres `/_not-found` **por suerte**, más un log de Vercel que
  corre con **1 worker** y por tanto no dice nada del caso paralelo. El implementador
  obedeció esa instrucción al pie de la letra y lo dejó escrito. **Queda anulada**: la
  observación de Vercel es la única que sigue en pie, y solo para Vercel.

  **Por qué bloquea.** Es el mismo daño que `F-SPEC-052-15`, en el mismo párrafo y bajo la
  misma palabra «Medido»: quien construya en local sin `APP_BASE_URL` verá, cinco veces de
  cada seis, una ruta que el runbook no nombra, en el bloque cuya razón de existir es
  *«decide dónde vas a ver el fallo»*. Y ahora es peor que en la 3.ª ronda, porque el
  párrafo **contrasta** explícitamente las dos mitades y el contraste es falso.

  **Arreglo propuesto** (una frase, sin tocar §0, ni los dos rótulos, ni ningún test — la
  simetría ahora **sí** está justificada por la medición):

  > Medido con `.next` borrado antes de cada corrida: falte la que falte, el build muere en
  > *Collecting page data* con **exit 1**, y **lo estable es el mensaje, no la ruta** —
  > `APP_BASE_URL no definida (ver .env.example): sin ella no hay enlaces válidos.` en un
  > caso, `DATABASE_URL no definida. Configúrala (ver .env.example).` en el otro. La ruta
  > que acompaña al mensaje **cambia de corrida a corrida** —se han observado `/_not-found`,
  > `/ayuda`, `/admin`, `/forgot-password`, `/cuenta`, `/cartera/importar`,
  > `/api/cron/refresh` y `/api/auth/[...nextauth]`— porque las páginas se recogen en
  > paralelo y revienta la primera que evalúe el módulo que falta. En el log de Vercel, que
  > construye con **un solo worker**, sí sale siempre la misma: fue `/_not-found` en el
  > PR #58.

  Si el implementador prefiere no fundir las dos mitades, la otra salida válida es **quitar
  `/_not-found`** de la mitad de `APP_BASE_URL` y dejar ahí también solo el mensaje. Lo que
  no vale es que el párrafo siga presentando una ruta local como *la* firma, ni que siga
  contrastando una mitad «variable» con otra «estable» que no lo es.

  **Nótese lo que NO hay que tocar**, y esta vez viene con la corrida que lo sostiene: la
  mitad de `DATABASE_URL` **ya es correcta** —literal byte a byte, rutas todas observadas,
  omisiones bien juzgadas— y `F-SPEC-052-15` está cerrado. §0 sigue verde
  (CA-4, `F-SPEC-052-14`). Los dos rótulos de CA-7 los clavan
  `tests/entornos-de-despliegue.test.ts:553-554`. Y la regla de las tres rondas anteriores
  sigue en pie, ahora también para el gate: **ninguna afirmación entra aquí sin más de una
  corrida detrás**.
...
- **F-SPEC-052-15** ✅ **CERRADO el 2026-08-25 por el implementador (4.ª ronda, autorizada
  ...
  **§3.2 nombra, bajo la palabra «Medido», la ruta en la que muere el build sin
  ...
  > Medido con `.next` borrado antes de cada corrida: sin `APP_BASE_URL` el build muere en
  > *Collecting page data* (`Failed to collect configuration for /_not-found`); sin
  > `DATABASE_URL`, **recogiendo `/api/auth/[...nextauth]`** (`DATABASE_URL no definida`).

  **La medición.** Cuatro `next build` sin `DATABASE_URL`, `.next` borrado antes de cada
  uno, mismo árbol, mismo commit, mismo entorno:

  | Corrida | Ruta que aparece en `Failed to collect …` |
  |---|---|
  | C | `/admin` |
  | C2 | `/cartera/importar` |
  | C3 | (solo capturé el mensaje: `DATABASE_URL no definida. Configúrala (ver .env.example).`) |
  | C4 | `/api/auth/[...nextauth]` |

  Y una quinta, de la 2.ª ronda, está escrita **en este mismo ledger** por el gate:
  `/api/cron/refresh`. Son **cuatro rutas distintas** para la misma causa. El motivo es
  mecánico y está en el propio log: `Collecting page data using 15 workers` — las páginas
  se recogen **en paralelo**, y revienta la primera que evalúe el módulo del cliente de BD.
  Cuál sea esa es una carrera. En Vercel el log del PR #58 dice `using 1 worker`, así que
  allí puede que sea estable; pero la frase no habla de Vercel: dice *«Medido con `.next`
  borrado antes de cada corrida»*, y describe justo la corrida local que no lo confirma.

  **Por qué bloquea, siendo un detalle.** Este bloque se presenta a sí mismo diciendo que
  la diferencia importa porque decide *«dónde vas a ver el fallo»*: es un **texto de
  diagnóstico**, y una firma de diagnóstico que no coincide con lo que el lector tiene
  delante hace exactamente el daño que esta spec existe para evitar. Quien vea
  `Failed to collect configuration for /admin` con `DATABASE_URL` ausente y lea aquí otra
  ruta puede concluir que su problema es otro. Es la **cuarta** afirmación de esta familia
  en la entrega (`-10`, `-11`, `-12`, y esta): las tres primeras se escribieron con la
  misma confianza y las tres eran falsas al medirlas. La diferencia entre «medido» y
  «medido una vez» es la que este ledger lleva tres rondas cobrando.

  **Nótese lo que NO está mal**, para que el arreglo no se pase de frenada: la mitad de
  `APP_BASE_URL` **sí** es estable —`/_not-found` en mis dos corridas de hoy, en la de la
  2.ª ronda y en el log real de Vercel del PR #58— y tiene explicación: `appBaseUrl()` se
  evalúa en el layout raíz, que importan **todas** las rutas, y `/_not-found` es la primera
  que se recoge. `DATABASE_URL` la lee un módulo que solo importan algunas, y de ahí la
  carrera. Esa mitad se queda como está.

  **Arreglo propuesto** (una frase, sin tocar §0 ni ningún test):

  > Medido con `.next` borrado antes de cada corrida: sin `APP_BASE_URL` el build muere en
  > *Collecting page data* (`Failed to collect configuration for /_not-found`); sin
  > `DATABASE_URL` muere en el mismo punto con `DATABASE_URL no definida (ver
  > .env.example)`, pero ahí **la ruta que aparece cambia de corrida a corrida** —`/admin`,
  > `/cartera/importar`, `/api/cron/refresh`, `/api/auth/[...nextauth]`…—: las páginas se
  > recogen en paralelo y revienta la primera que evalúe el cliente de BD. **Lo estable es
  > el mensaje, no la ruta.**

  Si el implementador prefiere no describir la carrera, la otra salida válida es **quitar
  la ruta** y dejar solo el mensaje. Lo que no vale es dejar una ruta concreta escrita como
...
- **F-SPEC-052-14** ✅ **CERRADO por el gate el 2026-08-25 (3.ª ronda): la leyenda de §0 es
  ...
  y sobre la que, correctamente, no tocó nada.

  La frase en discusión (§0, leyenda de `Preview + Production`): *«esta columna dice dónde
  vive, no qué pasa si falta. **Cuáles tumban el build lo dice §3.2**, que separa las que
  `next build` lee —esas sí dejan la PR sin preview— de las que solo hacen la preview más
  útil y cuya ausencia no impide construir.»*

  **Medido, cláusula por cláusula, contra el §3.2 de hoy y contra mis corridas:** (1) *«esta
  columna dice dónde vive, no qué pasa si falta»* — cierto, y es justo lo que arregló
  `F-SPEC-052-10`. (2) *«Cuáles tumban el build lo dice §3.2»* — cierto: §3.2 lo dice, y lo
  dice **bien**; su primer cajón es hoy exactamente `{DATABASE_URL, APP_BASE_URL}`, que es
  el conjunto que mi corrida **A** confirma cerrado. (3) *«separa las que `next build` lee
  … de las que solo hacen la preview más útil»* — cierto: §3.2 separa esos dos grupos, con
  esos dos rótulos, y ambas descripciones casan con los grupos 1 y 3. **Ninguna cláusula es
  falsa, y la leyenda no afirma exhaustividad**: «separa A de B» no dice «y no hay nada
  más». Además, la propia frase manda al lector a la autoridad, y la autoridad abre
  diciendo *«hay **tres** casos»*. La diferencia con `-10`, `-11` y `-12` es de naturaleza,
  no de grado: aquéllas eran **falsas al medirlas**; ésta es **cierta e incompleta**, y su
  incompletitud se resuelve siguiendo el enlace que ella misma pone.

  **Consecuencia operativa: no se toca §0.** CA-4 está cerrado y verde, y reabrir su prosa
  para ganar una enumeración que el lector obtiene un clic más allá arriesga un criterio ya
  ganado a cambio de nada. Si algún día el arquitecto quiere que la leyenda mencione el
  tercer caso, es un cambio cosmético de una frase, sin test detrás y **sin dueño asignado
  aquí**. La decisión del implementador de no tocarlo fue la correcta.

- **F-SPEC-052-16** 🟡 **RESIDUAL sin bloquear — dos imprecisiones menores que el gate deja
  anotadas para el arquitecto, NO para esta ronda (2026-08-25).** Ninguna de las dos es
  falsa en su parte load-bearing, y arreglarlas no cambia ningún CA.

  1. **`docs/despliegue.md` §3.2, nota final**: *«declara las cuatro claves **para que el
     job corra entero**»*. La razón no se sostiene del todo: el launcher del e2e
     (`tests/e2e/server.mjs:59-69`) **reinyecta las suyas** al proceso hijo —incluidas
     `AUTH_SECRET`, `AUTH_TRUST_HOST`, `APP_BASE_URL` y la `DATABASE_URL` del Postgres
     efímero—, así que `AUTH_SECRET` y `AUTH_TRUST_HOST` no las necesita ni el build (mi
     corrida **A**) ni el e2e. Están declaradas, no exigidas. La afirmación que sí importa
     —que el bloque es una **cota superior**— es cierta y se queda.
  2. **`.github/workflows/ci.yml:138`** sigue diciendo, en su comentario, que *«`next build`
     solo exige que estas variables EXISTAN»* refiriéndose a las cuatro. Es la **misma
     creencia** que originó `F-SPEC-052-12`, viva en otro fichero: el runbook ya dice que
     solo dos tumban el build y el workflow sigue insinuando que son cuatro. **Está fuera
     del alcance declarado de SPEC-052** (`ci.yml` no aparece en su tabla de ficheros
     tocados) y por eso no bloquea; pero es el sitio donde la próxima persona volverá a
     leer lo que esta spec acaba de desmentir.

- **F-SPEC-052-12** ✅ **CERRADO — confirmado por el gate del 2026-08-25 (3.ª ronda) con
  medición propia**, no leído del informe: corridas **A**, **B**, **C**, **D**, **E**, **F**
  y **G** de la tabla del veredicto. El cierre es genuino y no ha desplazado el defecto a
  ningún otro sitio: los dos rótulos de CA-7 siguen literales, §0 no se ha tocado y CA-4,
  CA-8, CA-9 y CA-10 siguen midiendo lo mismo. Texto original del cierre, del implementador:

- **F-SPEC-052-12 (cierre del implementador)** ✅ **CERRADO el 2026-08-25** (finding del gate del
  mismo día, 2.ª ronda). **§3.2 pasa de dos cajones a tres**, y los dos rótulos literales que
  fija CA-7 se quedan **palabra por palabra** donde estaban: lo que se ajusta es el **grupo**,
  no la aserción. Texto nuevo en `docs/despliegue.md` §3.2 (abreviado):

  > - **Obligatorias en Preview porque el build las lee** — `DATABASE_URL` y `APP_BASE_URL`,
  >   y solo esas dos. Si falta una, **`next build` falla y la PR se queda sin preview**. […]
  > - **Obligatorias en Preview aunque el build NO las lee** — `AUTH_SECRET` y
  >   `AUTH_TRUST_HOST`. Auth.js las lee **en cada petición**, no al construir […] y sin las
  >   dos `next build` termina **verde, exit 0** […]. La preview se construye, arranca y sus
  >   páginas públicas responden `200`. Lo que no se puede es **entrar** […]. Pero **no las
  >   busques en un log de build roto**, porque el build no se rompe.
  > - **Las que solo hacen la preview más útil** — `MARKETSTACK_API_KEY`. […]

  `ALLOW_MIGRATE=1` sale de los grupos a un párrafo propio. Su ausencia también deja la PR sin
  preview, pero el que sale con código `1` es `scripts/guard-migrate.mjs`, que corta el `&&`
  del `buildCommand` **antes** de que `next build` llegue a ejecutarse. Estaba dentro del
  primer grupo, bajo la frase *«si falta una, `next build` falla»*, y eso tampoco era cierto
  para ella: misma consecuencia visible, mecanismo distinto y log distinto.

  **Nada escrito sin medirlo.** `.next` **borrado antes de cada build**; el `.env.local` de
  este árbol solo define `VERCEL_OIDC_TOKEN` y no hay `.env`:

  | Corrida | Entorno | Resultado |
  |---|---|---|
  | **A** — `next build` | `DATABASE_URL` + `APP_BASE_URL`, **sin `AUTH_SECRET` ni `AUTH_TRUST_HOST`** | **verde, exit 0**, con el listado de rutas completo |
  | **B** — `next build` | sin `APP_BASE_URL` (las otras tres presentes) | **rojo, exit 1** — *Collecting page data* → `Failed to collect configuration for /_not-found`, causa `APP_BASE_URL no definida (ver .env.example)` |
  | **C** — `next build` | sin `DATABASE_URL` (las otras tres presentes) | **rojo, exit 1** — `DATABASE_URL no definida. Configúrala (ver .env.example).` recogiendo `/api/auth/[...nextauth]` |
  | **D** — `next start` sobre el build **A** | sin ninguna de las dos | `/login` **200**; `/dashboard` **307** → `/login`; `POST /api/auth/callback/credentials` **500** (*«There was a problem with the server configuration»*), log `UntrustedHost` |
  | **E** — `next start` sobre el mismo build | **con** `AUTH_TRUST_HOST=true`, **sin** `AUTH_SECRET` | `/login` y `/legal` **200**; el mismo `POST` **500**, log `MissingSecret: Please define a 'secret'` |

  **B** y **C** son el **control positivo** del build: el harness detecta fallos, así que el
  verde de **A** no es caché. **D** y **E** existen porque `UntrustedHost` se comprueba
  **antes** que el secreto y tapaba a `MissingSecret`: sin separarlas, la mitad de la
  afirmación se habría escrito sin medir. Corroborado en código: `grep -rn` de las dos claves
  sobre `src/` y `scripts/` devuelve **una sola línea, y es un comentario**
  (`src/lib/config/app-url.ts:6`); Auth.js las resuelve de `process.env` en petición
  (`node_modules/@auth/core/src/lib/utils/env.ts:44-50`).

  **Y una precisión de la misma familia, en el mismo §3.2** (nota final del bloque, también
  texto nuevo de esta entrega): decía *«lo que el build exige está congelado en el bloque
  `env` del job de CI»*. Ese bloque declara **cuatro** claves y el build solo exige **dos**,
  así que ahora dice que es una **cota superior**, no la lista exacta, y que la guardia
  sobreaproxima **a propósito** (F-SPEC-052-3): exige presencia en la tabla de §0 como
  `Preview + Production`, no que su ausencia rompa `next build`. Guardia y documento siguen
  sin decir lo mismo, que es lo correcto.

  **Lo que NO se ha tocado**: la tabla de §0 (las cuatro siguen `Preview + Production`, CA-8
  intacto), el vocabulario cerrado y sus tres valores, la spec, el ADR, `.env.example`,
  `src/` y **ni un fichero de `tests/`**. El diff de la corrección es `docs/despliegue.md` y
  este ledger. Suite completa: **1829/1829**, 114 ficheros.

  **Observación, no cambio** (ver también F-SPEC-052-14): la leyenda de §0 corregida por
  `F-SPEC-052-10` describe §3.2 como una separación de **dos** grupos. Sigue siendo cierta —
  §3.2 separa esos dos— pero ya no los enumera todos. §0 es CA-4 y está verde: no se toca.

  <details><summary>El finding original, tal como lo escribió el verificador</summary>

  **§3.2 promete un fallo de `next build` que no ocurre, para dos de las cuatro claves que
  enumera — y la leyenda corregida de §0 manda al lector justo ahí.** `docs/despliegue.md`
  §3.2:
...
- **F-SPEC-052-14** 🟢 **OBSERVACIÓN sin dueño, NO bloqueante — anotada por el implementador
  al cerrar `F-SPEC-052-12` (2026-08-25). No se ha tocado nada por ella.**

  Al partir §3.2 en **tres** grupos, la leyenda de §0 —corregida por `F-SPEC-052-10` y verde
  en CA-4— quedó describiendo §3.2 como una separación de **dos**: *«…que separa las que
  `next build` lee —esas sí dejan la PR sin preview— de las que solo hacen la preview más
  útil»*. **No es falsa**: §3.2 sí separa esas dos, y la remisión *«cuáles tumban el build lo
  dice §3.2»* —que es la afirmación que la leyenda hace en primera persona— sigue siendo
  exacta, porque el primer grupo es hoy justamente el conjunto que tumba el build. Lo que ya
  no hace es enumerar todos los casos que hay al otro lado del enlace.

  **Por qué no lo he arreglado**: §0 es CA-4, está cerrado y verde, y el encargo de esta
  ronda dice explícitamente que la tabla y la leyenda de §0 no cambian. Retocarlo sería
  ampliar el alcance por mi cuenta sobre un criterio ya verificado. Si el arquitecto quiere
  que la leyenda mencione el tercer caso, es un cambio de una frase en §0 y ningún test se
  entera (la aserción de CA-7 vive en §3.2, y CA-4 mide vocabulario y literales, no esta
  frase).
...
>
> El gate de la **2.ª ronda** cerró `F-SPEC-052-10` y `-11` pero dio **RED** con un solo
> finding: **`F-SPEC-052-12`**. La corrección anterior no había eliminado el defecto, lo había
> **desplazado**: la leyenda de §0 dejó de mentir y **delegó** en §3.2, y §3.2 —texto nuevo de
> esta entrega— atribuía un fallo de `next build` a `AUTH_SECRET` y `AUTH_TRUST_HOST`, que el
> build **no lee**. Está **CERRADO** arriba, con las cinco corridas de control pegadas.
>
> **Qué cambió y qué no.** Cambió **`docs/despliegue.md` §3.2 y nada más**: el bloque de los
> grupos de Preview y la nota final del mismo bloque. §0 **no se ha tocado** (CA-4 sigue como
> lo dejó la ronda anterior, y CA-8 sigue exigiendo las cuatro claves como
> `Preview + Production`, con razón). **Ni un fichero de `tests/`**, ni `.env.example`, ni
> `src/`, ni la spec, ni ningún ADR. Los **dos rótulos literales** que fija CA-7 y clava
> `tests/entornos-de-despliegue.test.ts:553-554` siguen **palabra por palabra**: lo que se
> reagrupó fueron las claves de debajo, no la aserción. Suite: **1829/1829**, 114/114.
>
> **La forma del arreglo, por si hay 4.ª ronda**: §3.2 tiene ahora **tres** grupos —lo que
> tumba el build (`DATABASE_URL`, `APP_BASE_URL`), lo que es obligatorio en Preview **sin**
> tumbar el build (`AUTH_SECRET`, `AUTH_TRUST_HOST`, porque sin ellas nadie entra) y lo que
> solo hace la preview más útil (`MARKETSTACK_API_KEY`)— más `ALLOW_MIGRATE=1` en un párrafo
> aparte, porque su fallo lo produce `guard-migrate.mjs` **antes** de que `next build` corra.
> Cada frase se midió: **cinco corridas**, tres `next build` con `.next` **borrado** y dos
> `next start` sobre el build verde. Están en la tabla de `F-SPEC-052-12`.
>
> **Para el verificador: solo hay que re-verificar CA-7.** Ningún otro CA ha sido tocado por
> esta ronda. Las celdas **Verif.** y **Estado** de la matriz **no se han tocado**: son suyas.
> Y si vas a medir, borra `.next` antes de cada build — sin eso el verde no significa nada.

> ### 2.ª ronda de corrección (2026-08-25)
>
...
**Estado real (2026-08-24, vigente salvo por las dos rondas de arriba)**: **implementación
