...
|---|---|---|---|---|
| CA-1 | `src/app/vigiladas/columnas-vigiladas.tsx` (celda «Activo»: `nota-senal` + `EnlacesSenal`), `src/app/_components/enlaces-senal.tsx` | tests/e2e/spec067-enlaces.spec.ts › «CA-1: dos señales distintas…» en tabla (1280) y tarjeta (390) | Iter. 2: e2e CA-1 tabla/tarjeta verde (403/403); fila-cuatro-combinaciones-{1280,390} revisadas: sin marca / sólo ✎ / sólo glifo enlace / ambas | ✅ |
| CA-2 | `src/lib/contexto/senal.ts` (`TEXTO_SENAL_NOTA`, `textoDeSenalDeNota`), `columnas-vigiladas.tsx` | tests/spec067-senales.test.ts › «CA-2…»; tests/e2e/spec067-enlaces.spec.ts › «CA-2: la señal de nota es información…» (tabla y tarjeta) | Iter. 2: unit + e2e CA-2 verdes; `role="img"`, «Tiene nota tuya» desde `senal.ts`, fuera de Tab, clic inerte | ✅ |
| CA-3 | `src/app/_components/enlaces-senal.tsx` (rama de 1 enlace: `<a target=_blank rel="noopener noreferrer">`) | tests/e2e/spec067-enlaces.spec.ts › «CA-3/CA-4…» (tabla y tarjeta: pestaña nueva interceptada en local, `window.opener` null, origen sigue en /vigiladas) | Iter. 2: e2e CA-3/CA-4 verde; código: `<a href target=_blank rel="noopener noreferrer">` con `href` canónico de `enlacesAbribles` | ✅ |
| CA-4 | `src/lib/contexto/senal.ts` (`destinoDeEnlace`) | tests/spec067-senales.test.ts › «CA-4…» (con etiqueta, sin etiqueta, etiqueta = dominio); tests/e2e/spec067-enlaces.spec.ts › «CA-3/CA-4…» (aria-label = title) | Iter. 2: unit CA-4 (etiqueta+dominio; sin etiqueta dominio 1 vez, nunca URL) + e2e aria-label = title | ✅ |
| CA-5 | `enlaces-senal.tsx` (botón `aria-haspopup="dialog"` + número; `<dialog>` modal por portal en `<body>`), `senal.ts` (`nombreDeSenalDeEnlaces`, `tituloDeCapaDeEnlaces`), `globals.css` (`dialog.enlaces-capa`) | tests/spec067-senales.test.ts › «CA-5…»; tests/e2e/spec067-enlaces.spec.ts › «CA-5: con varios…» (tabla y tarjeta) | Iter. 2: unit + e2e CA-5 verdes; capa-enlaces-390 revisada: título «Enlaces de Z9VARIOS · BME», 3 en orden, rótulo y dominio debajo | ✅ |
| CA-6 | `enlaces-senal.tsx` (todo cierre pasa por `dialog.close()` → evento `close` → desmonta y enfoca la señal) | tests/e2e/spec067-enlaces.spec.ts › «CA-6: la capa se comporta como la de editar» (Escape, Cerrar, activar enlace; tabla y tarjeta) | Iter. 2: e2e CA-6 verde (Escape, Cerrar, activar enlace → cierra y foco a la señal) | ✅ |
| CA-7 | `globals.css` (`.enlaces-senal` 22 px + `::after` bajo 720 px; `dialog.enlaces-capa` anclada abajo, 92dvh, overflow-y propio), `tests/e2e/geometria.ts` (sin cambios en M1–M5) | tests/e2e/spec067-enlaces.spec.ts › «CA-7: M1, M2, M3 a los ocho anchos, M5 por debajo de 720, y la tabla no crece»; «CA-7: con una lista larga de verdad… (M4)». Evidencia: `_qa/SPEC-067/geometria.txt`, `_qa/SPEC-067/m4-lista-larga.txt` | Iter. 2: e2e CA-7 ×2 verdes; geometria.txt y m4-lista-larga.txt reproducidos (tabla 933=933). M1 no mide la señal a 730–800 (tabla arrastrada, ADR-026 §4): salvedad aceptada | ⚠️ |
| CA-8 | `enlaces-senal.tsx` (foco al primer enlace tras `showModal`), `globals.css` (`:focus-visible` 2 px `--ember`) | tests/e2e/spec067-enlaces.spec.ts › «CA-8: se llega y se usa con teclado» (tabla y tarjeta) | Iter. 2: e2e CA-8 verde (Tab: señal antes que Editar, :focus-visible 2 px, Enter abre enlace/capa con foco en el 1.º) | ✅ |
| CA-9 | `enlaces-senal.tsx` (escuchador NATIVO con `stopPropagation` en la señal; `noSubir` en la capa por el portal) | tests/e2e/spec067-enlaces.spec.ts › «CA-9: activar la señal no hace nada más» (manejadores nativos en cada fila/tarjeta: 0 clics; URL y orden intactos) | Iter. 2: e2e CA-9 verde (0 clics en manejadores nativos de fila/tarjeta, URL y orden intactos, sin capa de edición) | ✅ |
| CA-10 | `src/lib/contexto/abribles.ts` (`esAbrible`, `enlacesAbribles`, reutiliza `ESQUEMAS_PERMITIDOS`) | tests/spec067-senales.test.ts › «CA-10…» (6+3 prohibidos, 5 permitidos, orden, sin segunda lista); tests/e2e/spec067-enlaces.spec.ts › «CA-10…» (Z9MALO sin señal de enlaces, Z9MEZCLA ofrece 2 de 3) | Iter. 2: unit CA-10 (9 no / 5 sí, reutiliza `ESQUEMAS_PERMITIDOS`) + e2e (Z9MALO sin señal, Z9MEZCLA 2 de 3) | ✅ |
| CA-11 | `abribles.ts`, `senal.ts`, `enlaces-senal.tsx` (sin `fetch`, sin favicon/prefetch) | tests/spec067-senales.test.ts › «CA-11…»; tests/e2e/spec067-enlaces.spec.ts › «CA-11: la app sigue sin visitar…» | Iter. 2: e2e CA-11 verde; grep: ni `fetch` ni prefetch ni favicon en los 3 módulos nuevos | ✅ |
| CA-12 | Diff sin `src/db/schema.ts`, sin migración, sin acción de servidor ni ruta (ver `git diff --name-only origin/main...HEAD`) | Gate: `git diff origin/main...HEAD` + `npm run db:scan` (sin cambios: las 2 de siempre, con waiver) | Iter. 2: `git diff --name-only origin/main...HEAD` (origin/main refrescado, rama al día): sin schema, migración, acción ni ruta; `db:scan` = las 2 de siempre con waiver | ✅ |
| CA-13 | `globals.css` (reposo `--fg-muted` / hover `--accent`+`--bg-step` / foco anillo `--ember`; `.nota-senal` a `max(12px,.8em)`), `tests/e2e/geometria.ts` (`medirContrasteDeControl`: lee `color(srgb 0–1 / a)` ×255, `rgb()` y, si no, resuelve por canvas; devuelve `lienzo`, `distanciaAlLienzo` y `zona` — corrige F-V1) | tests/e2e/spec067-enlaces.spec.ts › «CA-13: reposo, hover y foco…» (glifo ≥ 3:1: 7,43:1 sobre `zone-out`, **6,56:1 sobre el tinte `zone-buy`** de Z9VARIOS; y guarda nueva: la señal sobre fila en zona tiene un fondo a ≥ 10 unidades del lienzo — con el parser viejo daba 2 y el test caía en rojo). Capturas `_qa/SPEC-067/`: fila-cuatro-combinaciones-{1280,390}, senales-foco-{1280,390}, capa-enlaces-{1280,390}; `estados-y-contraste.txt` | Iter. 2: F-V1 corregido — `medirContrasteDeControl` parsea `color(srgb 0–1 / a)` ×255 (canvas como respaldo); e2e CA-13 verde con guarda de tinte ≥ 10; `estados-y-contraste.txt` reproducido idéntico: 6,56:1 sobre zone-buy rgb(29,43,34), 7,43:1 sobre zone-out; estados reposo/hover/foco distintos; suelo 12 px verde | ✅ |
| CA-14 | `tests/e2e/spec063-contexto.spec.ts`, `tests/spec063-vocabulario-y-ayuda.test.ts` (adaptados, ver Salvedades); `package.json`/`package-lock.json` 0.10.0 | Batería completa (unit + e2e) — ver handoff; `npm run version:check` tras commitear: 0.9.0 → 0.10.0 | Iter. 2: typecheck ✓, lint ✓, unit 143/2266 ✓, build + e2e 403/403 ✓ (VERCEL_ENV=development, APP_BASE_URL local), `version:check` 0.9.0→0.10.0 ✓ sobre árbol commiteado; adaptación SPEC-063 revisada: no afloja | ✅ |

## Veredicto del verificador
<!-- GREEN/RED + fecha + resumen. Lo escribe SOLO sdd-verificador. -->
**GREEN — 2026-09-23 (sdd-verificador, 2.ª verificación).** Juzgado sobre `8c6fbe1`
(rama al día con `origin/main`). Gates: typecheck ✓, lint ✓, unit 143 ficheros / 2266 ✓,
`npm run build` + e2e completa **403/403** ✓, `db:scan` sin cambios, `version:check`
0.9.0 → 0.10.0 ✓, `valida.mjs` OK sobre spec y ledger.

- **F-V1 (CA-13) — cerrado.** `medirContrasteDeControl` lee `color(srgb r g b / a)` en escala
  0–1 (×255) y resuelve cualquier otra sintaxis con un canvas; el test afirma además que el
  fondo bajo la señal de una fila en zona se aparta ≥ 10 unidades del lienzo. La evidencia
  reproducida sale idéntica: glifo a 6,56:1 sobre el tinte `zone-buy` rgb(29,43,34) y 7,43:1
  sobre `zone-out`, que casa con la captura.
- **F-V2 (proceso) — cerrado.** Frontmatter en la línea 1, las 14 filas del implementador en la
  matriz, `valida.mjs` OK. Cifra de `.table-scroll` reconciliada (933/933).

13 CA ✅ y CA-7 ⚠️ con salvedad aceptada: M1 no mide la señal dentro de la tabla arrastrada a
730–800 px (ADR-026 §4, por diseño); ahí se mide el contenedor y el ancho del contenido, idéntico
con y sin señales. Salvedad menor, no bloqueante: en `estados-y-contraste.txt` la lectura de
«foco» en tabla se toma a mitad de la transición del hover (alfas 0,945/0,835); el anillo —lo
que distingue el foco— sí es `solid`.

Veredicto anterior (se conserva):
...
| CA | Captura / medida en `_qa/SPEC-067/` |
|---|---|
| CA-1, CA-13 | `fila-cuatro-combinaciones-1280.png`, `fila-cuatro-combinaciones-390.png` |
| CA-8, CA-13 | `senales-foco-1280.png`, `senales-foco-390.png` |
| CA-5, CA-13 | `capa-enlaces-1280.png`, `capa-enlaces-390.png` |
| CA-7 | `geometria.txt`, `m4-lista-larga.txt` |
| CA-13 | `estados-y-contraste.txt` |
