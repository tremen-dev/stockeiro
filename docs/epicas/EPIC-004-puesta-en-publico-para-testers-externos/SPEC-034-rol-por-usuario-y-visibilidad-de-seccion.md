...
mínimo que bloquea publicar**. Además es prerrequisito de **SPEC-037**, que necesita el rol
para la segunda condición de la identidad del operador (**ADR-023** pto. 9).
...
- **Usuario `completo` (hoy, el autor)**: no pierde nada. Su cuenta ya existe y la migración
  la deja donde estaba (**ADR-021** pto. 8). Sigue viendo las cuatro secciones.
...
- **CA-1 (La columna existe, con dominio cerrado y por migración — ADR-021 pto. 1, RI-01).**
  ...
  entonces existe `role`, es `NOT NULL`, y la base **rechaza** un valor distinto de
  `'tester'` o `'completo'` (un `UPDATE` a `'admin'` falla). La migración **no borra, no
  renombra y no estrecha** nada (**RI-01**): el código anterior a esta spec sigue
  funcionando contra el esquema nuevo.

- **CA-2 (Las cuentas que ya existían siguen siendo `completo` — ADR-021 pto. 8).**
  Dada una base con cuentas creadas **antes** de la migración,
  cuando la migración se aplica,
  entonces **todas** quedan con rol `completo` y **ninguna** pierde acceso a ninguna
  sección. Verificable aplicando la migración sobre una base sembrada previamente.
...
- **CA-4 (La decisión de visibilidad es una función pura y exhaustiva — ADR-021 pto. 5).**
  ...
  cuando se evalúa el producto cartesiano de los dos roles por las cinco secciones
  (panel, vigiladas, avisos, cartera, importar),
  entonces `tester` obtiene **verdadero para panel, vigiladas y avisos** y **falso para
  cartera e importar**; `completo` obtiene verdadero en las cinco. La función **no** lee la
  base, **no** importa Next y se prueba sin levantar nada.

- **CA-5 (El menú enseña exactamente lo que el rol permite — CE-2).**
  Dada una sesión con rol `tester` y otra con rol `completo`,
  ...
  con su contador de no leídos intacto (SPEC-007 CA-10); la de `completo` contiene las
  cuatro. El menú **consulta la misma función de CA-4**, no una lista propia.
...
- **CA-9 (El panel del tester no ofrece lo que no puede abrir — épica, §Alcance).**
  ...
  `/cartera` en ningún texto; cuando lo visita un `completo`, aparecen las **tres** tarjetas
  de hoy, intactas.
...
- **CA-10 (Degradar y promover surten efecto sin volver a iniciar sesión — ADR-021 pto. 4).**
  ...
  siguiente petición **sí** accede y **sí** ve el enlace, también con la misma cookie.
  Verificable en Playwright con un solo contexto de navegador.
...
- **CA-13 (El rol no relaja ni sustituye a RN-01 — ADR-021 pto. 9).**
  Dados dos usuarios `completo` con datos propios,
  cuando cada uno navega por todas las secciones,
  entonces **ninguno ve ni un dato del otro**, exactamente igual que antes de esta spec:
  el rol **no** es un permiso sobre datos ajenos. Sigue verde CA-6 de SPEC-001.
...
- **`users`** gana **`role`** (`text NOT NULL`, dominio `'tester' | 'completo'` impuesto con
  `CHECK`, sin `enum` de Postgres — **ADR-021** pto. 1). La migración añade con
  `DEFAULT 'completo'` y a continuación fija el default en `'tester'` (**ADR-021** pto. 8).
...
añaden a `docs/fundacion/dominio.md` los términos **Rol de cuenta** (`tester` / `completo`)
y **Sección**.
...
- **Un tercer rol de administrador.** Quién opera la app se resuelve **fuera del rol**, en
  **ADR-023**, y su pantalla es **SPEC-037**. Aquí el enum tiene dos valores y punto.
...
- **F-SPEC-034-2 (orden de despliegue).** Entre el momento en que la migración corre y el
  momento en que el código nuevo está vivo, todas las cuentas son `completo` y nadie ve
  nada distinto. Es la propiedad que hace este despliegue aburrido, y es intencionada.
...
3. **Toda cuenta nueva nace `tester`, incluida la tuya si te registras otra vez.** Es lo que
   quiere la épica, pero tiene una consecuencia doméstica: si algún día borras tu cuenta
   (**SPEC-036**) y vuelves a registrarte, entras como tester y necesitarás un `UPDATE` para
   recuperar Cartera. **ADR-023** convierte eso en un asunto serio, porque la identidad del
   operador exige rol `completo`: por eso **ADR-022** pto. 8 prohíbe que la cuenta del
   operador se borre desde la app.
...
7. **Secuencia.** Esta spec va **primera** de EPIC-004: **SPEC-037** depende de su columna
...
8. **Aprobación**: la spec queda en **`borrador`** y **no la firmo yo**. **ADR-021** nace
